...
import { makeProposeInstrumentationTool, type InstrumentationSink } from './proposeTool.js'
import type { InstrumentationPlan } from './instrumentSchema.js'
...
export type InstrumentEvent =
  ...
  | { type: 'submitted' }
  | { type: 'completed'; result: InstrumentResult }
...
export async function runInstrument(options: RunInstrumentOptions): Promise<InstrumentResult> {
  ...
  try {
    const q = query({
      prompt:
        'Begin /instrument now. Follow the method in your system prompt and call proposeInstrumentationPlan once you have a complete plan.',
      options: {
        cwd,
        model: 'sonnet',
    ...
    const result: InstrumentResult = {
      ...
      error:
        'The agent ended without submitting an instrumentation plan. Likely it hit maxTurns or got confused — re-run with a higher maxTurns or in a smaller package.',
      sessionId,
      durationMs,
    }
    onEvent?.({ type: 'completed', result })
