...
const COMMAND_HELP = [
  ...
  '             [--out PATH]',
  '    holostaff import NAME|PATH|URL                    Import a published journey map',
  '                                                      instead of scanning (try: import opnform)',
  '    holostaff import                                  List available presets',
  '    holostaff deploy [--dry-run] [--force]            Open a deploy PR for the bound source',
  '             [--local] [--merged]                     --local: write the branch here, no GitHub App',
  '                                                      --merged: mark a --local deploy merged',
...
export function formatWhoami(): { ok: boolean; text: string } {
  ...
  return {
    ...
    text: [
      `Signed in.`,
      `Account:   ${auth.email ? `${auth.email} (uid ${auth.userId ?? 'unknown'})` : `${auth.userId ?? '(unknown)'} (email not recorded, run \`holostaff login\` to refresh)`}`,
      `Workspace: ${auth.workspaceName ? `${auth.workspaceName} (${auth.workspaceId ?? 'unknown'})` : (auth.workspaceId ?? '(unknown)')}`,
      `Backend:   ${auth.baseUrl}`,
